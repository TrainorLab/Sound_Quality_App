...
class ScreenManagerApp(App):
    def build(self):
        self.store = JsonStore('responses.json')
        self.slider1 = Slider(min=1, max=5, value=3)
        self.slider2 = Slider(min=1, max=5, value=3)
        self.label1 = Label(text='Please rate the previous 5 minutes on "naturalness" on a scale from 1-5', size_hint_y=None, height=30)
        self.label2 = Label(text='Please rate the previous 5 minutes on "sound quality" on a scale from 1-5', size_hint_y=None, height=30)
        ...
        self.main_screen_layout.add_widget(self.label1)
        self.main_screen_layout.add_widget(self.slider1)
        self.main_screen_layout.add_widget(self.label2)
